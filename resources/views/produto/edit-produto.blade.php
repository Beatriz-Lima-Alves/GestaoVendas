        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="linha">
                        <p class="t-20">{{ __("Editar Produto") }}</p>
                    </div>
                    <hr style="margin-bottom: 20px;">
                    <form action="{{route('produto.update', $produto->id)}}" class="text-black" method="post">
                        @csrf
                        @method('PUT')

                        <div>
                            <x-input-label for="nome" :value="__('Nome')"  class="text-black"/>
                            <x-text-input id="nome" class="block mt-1 w-full text-black" type="text" name="nome" :value="$produto->nome" required autofocus />
                        ...
                        <div style="display: flex; margin-top: 1%">
                            ...
                            <div  style="margin-right: 20px;">
                                ...
                                <select id="id_fornecedor" name="id_fornecedor" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    ...
                                    @foreach ($fornecedors as $fornecedor)
                                        <option value="{{ $fornecedor->id }}" {{ $produto->id_fornecedor == $fornecedor->id ? 'selected' : '' }}>{{ $fornecedor->nome }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div  style="margin-right: 20px;">
                                <x-input-label for="cod_produto" :value="__('Código do produto')"  class="text-black"/>
                                <x-text-input id="cod_produto" class="block mt-1 w-full text-black" type="text" name="cod_produto" :value="$produto->cod_produto" required autofocus />
                            ...
                            <div>
                                ...
                                <x-text-input id="preco" class="block mt-1 w-full text-black" type="number" name="preco" :value="$produto->preco" step="0.01" required autofocus />
